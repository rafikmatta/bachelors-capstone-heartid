    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying standalone applications, it is possible 
              to run the shell script file run_dotnet.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    NOTE: dotnet.ctf will be present only if you have chosen 
    ...
    All Platforms:
        * dotnet.ctf (if present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * dotnet.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * dotnet (application created by MATLAB Compiler)
        * run_dotnet.sh (shell script that can be run to 
    ...
    All Platforms:
        ...
        * dotnet.dll (shared library, file extension varies by 
          platform)
        * dotnet.h (library header file)
    ...
    Windows:
        * dotnet.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)    
        * dotnet.xml (documentation files)
        * dotnet.pdb (program database file - if DEBUG option 
          selected)
        * dotnet.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    All Platforms: 
        * dotnet.jar (Java class files and compiled M-code)
    ...
    Windows:
        * dotnet.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * dotnet_0_0.dll (COM component)
        * dotnet.xla (optionally created and placed in
          D:\School_Work\Design_Project\Conversion_individual\dotnet directory by the user)  
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
